...
(function() {
  ...
  Nimbus.Auth.Firebase = (function() {
    ...
    obj.init_workspace = function(callback) {
        ...
        if (!data) {
          ...
          if (localStorage['last_opened_workspace']) {
            if (data[localStorage['last_opened_workspace']]) {
              workspace = Nimbus.realtime.c_file = data[localStorage['last_opened_workspace']];
            }
          } else {
      ...
      return server.child('workspaces').on('value', function(res) {
        ...
        data = res.val();
        return Nimbus.realtime.app_files = Object.keys(data).length ? obj_to_array(data) : [];
      });
    };
    ...
    obj.authenticate_firebase = function(p, d) {
      ...
      self = this;
      provider = d ? d.provider : Nimbus.Auth.setting.provider;
  ...
  Nimbus.Client.Firebase = (function() {
    ...
    client.register_user = function(email, callback) {
      ...
      return this.createUser(data, callback);
    };
    client.createUser = function(data, callback) {
      var createUrl, email, password, ref;
      email = data.email;
      password = data.password;
      ref = server;
      ...
      window.$.ajax({
        ...
        success: function(data) {
          if (data.uid) {
            ref.child('/users/' + email.replace('.', ',')).set(data, function(res) {
    ...
    client.add_share_user_real = function(email, callback) {
      ...
      return server.child('users/' + email.replace('.', ',')).once('value', function(res) {
        var data, item;
        data = res.val();
        if (data) {
          item = {
            ...
            "email": email,
            "name": email,
            permissionId: data.uid
          };
          return add_share(item, callback);
        } else {
          return self.register_user(email, function(data) {
            item = {
              displayName: data.uid,
              pid: data.uid,
              id: data.uid,
    ...
    client.remove_share_user_real = function(email, callback) {
      var allows, node, obj, remove_allow, workspace;
      workspace = Nimbus.realtime.c_file;
      node = "workspaces/" + workspace.id + "/users/";
      allows = "workspaces/" + workspace.id + "/allows/";
      obj = {};
      ...
      return remove_allow = function(obj) {
        return server.child(allows).transaction(function(data) {
          if (!data) {
            data = {};
          }
