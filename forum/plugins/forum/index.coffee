...
defineController = (components) -> 
  ...
  foundryModule.controller('ForumController',['$scope', '$rootScope', '$foundry', '$filter' , ($scope, $rootScope, $foundry, $filter)->
    ...
    ###
    window.topic_model = $scope.topic_model
    window.link_model = $scope.link_model
    window.comment_model = $scope.comment_model
    ...
    $scope.currentSearchPage = 1
    $scope.lastSearch = ""
    #window.topic_model.create({ "name" : "How to make a great coffee", "link": "http://google.com" })
    
    window.controller["ForumController"] = $scope
    ...
    $scope.get_user_pic = (id)->
      if id is 10000
        return 'assets/img/forum_avatar.jpg'

      # get email
      email = if foundry._user_list[id] then foundry._user_list[id].email else id 
    ...
    $scope.get_current_user_pic = ()->
      if foundry._current_user
        $scope.get_user_pic(foundry._current_user.id) or 'assets/img/photo.jpg'
      else 
        ''
